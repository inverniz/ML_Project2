...
def mask_to_submission_strings(img_number, pred, img_size=608):
    """Yield the strings that goes into the submission file for a particular image
    Is given the img_number, its prediction per patch and its size"""
    patch_size = 16
    idx = 0
...
def create_submission(test_path, filename, model):
    """Create a submission for a given model"""
    
    with open(filename, 'w') as f:
        f.write('id,prediction\n')
        for i in range(1, 51):
            print("Predicting %d"%i)
            im = mpimg.imread(test_path +'/test_%d/test_%d.png'%(i,i))
